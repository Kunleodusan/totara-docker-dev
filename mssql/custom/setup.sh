...
timeout=$(($(date +%s) + 120))
until /opt/mssql-tools/bin/sqlcmd -No -H localhost -U sa -P "$SA_PASSWORD" -Q "select 'hello'" 2>/dev/null || [[ $(date +%s) -gt $timeout ]]; do
...
/opt/mssql-tools/bin/sqlcmd -No -S localhost -U SA -P "$SA_PASSWORD" -d master -i /custom/setup.sql